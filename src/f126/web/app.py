...
def _api_router(db_conn_factory: Callable[[], Any] | None) -> APIRouter:
    ...
    @router.get("/sessions/{session_id}/debrief", response_model=None)
    async def get_debrief(session_id: SessionId) -> Any:
        """The stored post-session debrief: prose plus the fact sheet it was written from.

        Read-only, like everything else here. Debriefs are generated by the serve process
        when a session finishes and by `f126 debrief <id>`; there is no route that makes
        one, because adding a POST for the convenience of a button would cost the invariant
        that this whole surface can only be read from.

        404 when the session has no debrief — which is the normal state of a session that
        was just recorded, not an error.
        """
        row = await _query(db_conn_factory, "debrief", int(session_id))
        if row is None:
